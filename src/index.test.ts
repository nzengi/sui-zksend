...
export const DEMO_BEAR_CONFIG = {
  packageId:
    "0xab8ed19f16874f9b8b66b0b6e325ee064848b1a7fdcb1c2f0478b17ad8574e65",
...
describe("Contract links", () => {
  ...
  test("reclaim links", async () => {
    ...
    const { digest: claimDigest } = await lostLink.claimAssets(
      keypair.toSuiAddress(),
      {
        reclaim: true,
...
describe("Non contract links", () => {
  test("Links with separate gas coin", async () => {
    ...
    const claimTx = await claimLink.claimAssets(
  ...
  test("create link with minted assets", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const bear = tx.moveCall({
        ...
        arguments: [
          tx.pure.string(
            `A happy bear - ${Math.floor(Math.random() * 1_000_000_000)}`
          ),
        ],
      });
  ...
  test("list created links", async () => {
    ...
    expect(createdLinks.links[0]?.link.address).toEqual(
      link.keypair.toSuiAddress()
    );
