...
`);
export async function listCreatedLinks({ address, cursor, network, contract = getContractIds(network), fetch: fetchFn, ...linkOptions }) {
    ...
    const links = (await Promise.all(transactionBlocks.nodes.map(async (node) => {
        ...
        const fn = commands.find((command) => command.MoveCall?.package === packageId &&
